...
from tasks import TASKS, get_tasks_for_mode, save_tasks
...
class AutoScriptGUI:
    ...
    def build_ui(self):
        ...
        self.window_entry = ttk.Entry(topbar, width=22)
        self.window_entry.pack(side="left", padx=(0, 8))
        ...
        self.window_entry.insert(0, config.TARGET_WINDOW_TITLE or "")
    ...
    def on_mode_selected(self, event=None):
        ...
        preset_tasks = get_tasks_for_mode(mode)
        self.task_listbox.delete(0, tk.END)
        for index, task in enumerate(preset_tasks):
            task["enabled"] = True
            label = f"[✓] {index + 1}. {task.get('description', task.get('template', 'unknown'))}"
            self.task_listbox.insert(tk.END, self._append_task_drag_handle(label))
        if preset_tasks:
            self.selected_task_index = 0
    ...
    def on_window_selected(self, event=None):
        ...
        self.window_entry.insert(0, value)
    ...
    def refresh_window_list(self):
        ...
        if titles:
            ...
            self.window_entry.delete(0, tk.END)
            self.window_entry.insert(0, titles[0])
        else:
    ...
    def _ensure_group_metadata(self):
        ...
        if not self.group_order:
            default_group_id = self._ensure_group_exists(group_id="group_default", group_name="默认分组", parent_id=None)
            self.group_order = [default_group_id]
    ...
    def start_script(self):
        ...
        window_title = self.window_var.get().strip() or self.window_entry.get().strip()
